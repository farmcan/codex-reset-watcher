...
No direct project had both a public evidence dashboard, official X API cursors, an event-by-event audit, independent-rumor grouping, visible health and server-side email. That combination is why a separate repository is justified.
...
| --- | --- | --- |
| [`codex-reset.com`](https://codex-reset.com/) | Polished tracker, public APIs, personal reset calculator and clear caveats | Broad feature surface. This dashboard stays focused on “current signal → evidence → audited history → health”. |
...
| --- | --- | --- |
| [`semichcsc-byte/ha-countdown-card`](https://github.com/semichcsc-byte/ha-countdown-card) | Count-up mode, progress bar, and phase-aware styling | The clock counts up from the latest confirmed reset benefit, labels hard versus banked, and shows progress against the audited historical median gap. |
...
- Page order: current decision, audited proof, live signals, source credibility, watcher health.
- No exact next-reset probability until a complete prediction ledger provides false positives and calibration.
- The primary count-up clock uses the median interval across all audited reset benefits as a descriptive yardstick, not a forecast or action trigger. Time since the latest hard reset remains visible separately.
